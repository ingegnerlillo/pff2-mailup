...
class MailUpClient {
    ...
    function retreiveAccessTokenWithCode($code) {
        ...
        if ($code != 200 && $code != 302) throw new \Exception("Authorization error", $code);
    ...
    function callMethod($url, $verb, $body = "", $contentType = "JSON", $refresh = true)
    {
        ...
        } else if ($code == 401 && $refresh == false) throw new \Exception("Authorization error", $code);
        else if ($code != 200 && $code != 302) throw new \Exception("Unknown error", $code);
    ...
    function getLists(){
        $url = $this->consoleEndpoint . "/Console/User/Lists";
        $result = $this->callMethod($url, "GET", null, "JSON");
        $lists = json_decode($result);
        return $lists->Items;
    }

    function getGroups($idList){
        $url = $this->consoleEndpoint . "/Console/List/".$idList."/Groups";
        $result = $this->callMethod($url, "GET", null, "JSON");
        $lists = json_decode($result);
        return $lists->Items;
    }
